      ...
      {project && (
        ...
        >
          <motion.div 
            ...
            className="relative w-full max-w-3xl bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col"
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-white/90 border border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-white flex items-center justify-center transition-colors cursor-pointer shadow-xs"
            ...
            <div className="overflow-y-auto">
              ...
              <div className="p-6 sm:p-8 space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.services.map((srv, idx) => (
                      <span 
                        key={idx} 
                        className="px-3 py-1 rounded-lg text-xs font-medium bg-slate-100 border border-slate-200 text-slate-700"
                      >
                        {srv}
                      </span>
                    ))}
                ...
                {/* Concept Note Notice */}
                <div className="p-4 rounded-xl bg-sky-50 border border-sky-200 flex items-start gap-3 text-xs text-slate-600">
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <MagneticButton
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full text-sm font-bold text-white bg-gradient-to-r from-sky-500 to-indigo-600 hover:shadow-[0_6px_20px_rgba(2,132,199,0.35)] transition-all cursor-pointer"
                  >
                  ...
                  <button
                    onClick={onClose}
                    className="w-full sm:w-auto px-5 py-3 rounded-full text-xs font-semibold text-slate-500 hover:text-slate-900 transition-colors cursor-pointer text-center"
                  >
                    Close Preview
                  </button>
                </div>
